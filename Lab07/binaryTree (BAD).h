...
class binaryTreeType {
 ...
 private:
  ...
  int nodeCountLoop(nodeType<elemType> *p, int &count) const;
  // Function to determine the number of nodes in
  // the binary tree to which p points.
  // Postcondition: The number of nodes in the binary
  //               tree to which p points is returned.

  int leafCountLoop(nodeType<elemType> *p, int &count) const;
  // Function to determine the number of nodes in
  // the binary tree to which p points.
  // Postcondition: The number of nodes in the binary
  //               tree to which p points is returned.
...
template <class elemType>
int binaryTreeType<elemType>::nodeCountLoop(nodeType<elemType> *p, int &count) const {
  if (p != nullptr) {
    nodeCountLoop(p->lLink, count);
    nodeCountLoop(p->rLink, count);
    count++;
  }
  return 0;
}
...
template <class elemType>
int binaryTreeType<elemType>::leafCountLoop(nodeType<elemType> *p, int &count) const {
  if (p != nullptr) {
    if(p->lLink == nullptr && p->rLink == nullptr)
      count++; 
    leafCountLoop(p->lLink, count);
    leafCountLoop(p->rLink, count);
  }
  return 0;
}
