...
class bSearchTreeType : public binaryTreeType<elemType> {
   public:
    ...
    bool search(const elemType &searchItem, int &compareCount) const;
...
template <class elemType>
bool bSearchTreeType<elemType>::search(const elemType &searchItem, int &compareCount) const {
    nodeType<elemType> *current;
    bool found = false;
    compareCount = 0;
    ...
    if (this->root == nullptr)
    {
        ...
        while (current != nullptr && !found) {
            if (compareCount++ && current->info == searchItem)
            {
                found = true;
            }
            else if (compareCount++ && current->info > searchItem)
            {
                current = current->lLink;
                compareCount++;
            }
...
template <class elemType>
bool bSearchTreeType<elemType>::insert(const elemType &insertItem) {
